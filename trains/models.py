...
class Train(models.Model):
    '''Поезд'''
    name = models.CharField(max_length=30, verbose_name='Номер поезда', unique=True)
    travel_time = models.PositiveSmallIntegerField(verbose_name='Время в пути')
    from_city = models.ForeignKey(City, on_delete=models.CASCADE, verbose_name='Откуда', related_name='from_city_set')
    to_city = models.ForeignKey(City, on_delete=models.CASCADE, verbose_name='Куда', related_name='to_city_set')
    ...
    def __str__(self):
        return f'Поезда №{self.name} из города {self.from_city}'
